...
class Convolution(Module):
    ...
    def __init__(self, filtersize=(5,5,3,32), stride = (2,2), padding='valid'):
        '''
    ...
    def forward(self,X,padding=False,lrp_aware=False):
        ...
        if self.padding == 'same':
            ht = int(self.W.shape[0] / 2)
            wt = int(self.W.shape[1] / 2)
            X = np.pad(X, ((0,0), (ht,ht), (wt,wt), (0,0)), 'constant', constant_values=(0.,)) # FOR LENET MODELS
    ...
    def _simple_lrp(self,R, X, Y):
        ...
        self.X = X
        self.Y = Y
        self.lrp_aware = False

        N,Hout,Wout,NF = R.shape
        hf,wf,df,NF = self.W.shape
        hstride, wstride = self.stride
        # if self.padding == 'same':
        #     Xs = np.zeros(X.shape[0]+)
        Rx = np.zeros_like(self.X, dtype=np.float)
