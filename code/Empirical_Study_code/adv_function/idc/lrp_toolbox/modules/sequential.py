...
class Sequential(Module):
    ...
    def lrp(self, R, inp_trace, only_last_layer=False, lrp_var=None, param=None):
        '''
        ...
        temporary data
        '''
        inp_trace = inp_trace[::-1]
        all_layer_relevances = []
        for layer_num, m in enumerate(self.modules[::-1]):
            R = m.lrp(R, np.expand_dims(inp_trace[layer_num+1], axis=0), np.expand_dims(inp_trace[layer_num], axis=0), lrp_var,param)
            all_layer_relevances.append(R)
        if only_last_layer:
            return R, all_layer_relevances
        else:
