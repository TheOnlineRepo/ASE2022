    ...
    def _simple_lrp_slow(self,R):
        ...
        for i in range(Hout):
            for j in range(Wout):
                ...
                Zs = Z.mean(axis=(1,2),keepdims=True)
    ...
    def _alphabeta_lrp_slow(self,R,alpha):
        ...
        for i in range(Hout):
            for j in range(Wout):
                ...
                if not alpha == 0:
                    Zp = Z * (Z > 0)
                    Zsp = Zp.mean(axis=(1,2),keepdims=True) + 1e-16 #zero division is quite likely in sum pooling layers when using the alpha-variant
                ...
                if not beta == 0:
                    Zn = Z * (Z < 0)
                    Zsn = Zn.mean(axis=(1,2),keepdims=True) - 1e-16 #zero division is quite likely in sum pooling layers when using the alpha-variant
        ...
        for i in range(Hout):
            for j in range(Wout):
                ...
                if alpha * beta != 0 : #the general case: both parameters are not 0
                    Zp = Z * Zplus
                    Zsp = Zp.mean(axis=(1,2),keepdims=True)+ 1e-16
                    ...
                    Zsn = Zn.mean(axis=(1,2),keepdims=True) - 1e-16
