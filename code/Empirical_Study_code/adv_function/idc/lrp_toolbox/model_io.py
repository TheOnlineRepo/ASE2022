...
import pickle
import numpy as np
from adv_function.idc.lrp_toolbox.modules import Sequential,Linear,Tanh,Rect,SoftMax,Convolution,Flatten,SumPool,MaxPool,Add,AveragePool
import json
        ...
        with open(path, 'r') as f:
            ...
            while len(line) > 0:
                if line.startswith(Linear.__name__): # @UndefinedVariable import error suppression for PyDev users
                    ...
                    '''
                    _,m,n,act = line.split();   m = int(m); n = int(n)
                    layer = Linear(m,n)
                    layer.act = act
                    ...
                    _,h,w,d,n,s0,s1,padding = line.split()
                ...
                elif line.startswith(AveragePool.__name__): # @UndefinedVariable import error suppression for PyDev users
                    '''
                    Format of sum pooling layer
                    SumPool <mask_heigth> <mask_width> <stride_axis_0> <stride_axis_1>
                    '''
                    ...
                    _,h,w,s0,s1 = line.split()
                    h = int(h); w = int(w); s0 = int(s0); s1 = int(s1)
                    layer = AveragePool(pool=(h,w),stride=(s0,s1))
                    modules.append(layer)
                    c+=1 # one line of parameterized layer description
                ...
                elif line.startswith(Add.__name__):
                    modules.append(Add()) ; c+=1 #one line of parameterless layer description
                    print('Add OK')
                else:
...
def write(model, path, num_channels = None, fmt = None, only_last_layer=False):
    '''
    ...
    _write_as[fmt](model, path, num_channels, only_last_layer)
...
def _write_keras_txt(model, destination_path, num_channels, only_last_layer):
    ...
    if only_last_layer:
        if json_obj['config']['layers'][-1]['class_name'] == 'Dense':
            start_layer = -1
            prev_layer = model.layers[-2]
        else:
            start_layer = -2
            prev_layer = model.layers[-3]
    else:
        start_layer = 0
        prev_layer = None
    ...
    for layer, json_elem in zip(model.layers[start_layer:], json_obj['config']['layers'][start_layer:]):
        if json_elem['class_name'] == 'Activation':
            ...
            txtfile.write('Linear ' + str(prev_layer.output_shape[-1]) + ' ' + str(layer.output_shape[-1]) + ' ' + json_elem['config']['activation'])
        elif json_elem['class_name'] == 'Conv2D':
            txtfile.write('Convolution ' + str(json_elem['config']['kernel_size'][0]) + ' ' + str(json_elem['config']['kernel_size'][1]) + ' ' + prev_filter_num + ' ' + str(json_elem['config']['filters']) + ' ' + str(json_elem['config']['strides'][0]) + ' ' + str(json_elem['config']['strides'][1]) + ' ' + str(json_elem['config']['padding']))
        ...
        elif json_elem['class_name'] == 'AveragePooling2D':
            print(layer.output_shape)
            txtfile.write('AveragePool ' +  str(json_elem['config']['pool_size'][0]) + ' '\
                + str(json_elem['config']['pool_size'][1]) + ' ' \
                + str(json_elem['config']['strides'][0]) + ' ' \
                + str(json_elem['config']['strides'][1]))
        elif json_elem['class_name'] == 'Flatten':
            txtfile.write(json_elem['class_name'])
        elif json_elem['class_name'] == 'Add':
            txtfile.write(json_elem['class_name'])
        else:
