...
def exp(coverage, use_adv, std=0, k=1, k_bins=1000):
    ...
    df['rate'] = rate
    if coverage == 'kmnc':
        df.to_csv('./output_svhn/{}_{}_k_bins_{}.csv'.format(dataset, coverage, k_bins))
    elif coverage == 'nbc':
        df.to_csv('./output_svhn/{}_{}_std_{}.csv'.format(dataset, coverage, std))
    elif coverage == 'snac':
        df.to_csv('./output_svhn/{}_{}_std_{}.csv'.format(dataset, coverage, std))
    elif coverage == 'tknc':
        df.to_csv('./output_svhn/{}_{}_k_{}.csv'.format(dataset, coverage, k))
...
def exp_nac(use_adv, t):
    ...
    df.to_csv('./output_svhn/{}_nac_t_{}.csv'.format(dataset, t))
...
def exp_deep_metric(use_adv):
    ...
    if use_adv:
        ...
        dataset = 'svhn'
    df.to_csv('./output_svhn/{}_deep_metric.csv'.format(dataset))
